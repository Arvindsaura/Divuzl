...
const Footer = () => {
  ...
  <div className="w-[90vw] max-w-7xl mx-auto py-16 flex flex-col gap-14">
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6 sm:gap-8">
        {socials.map((s, i) => (
          ...
          >
            <span className="text-4xl sm:text-5xl flex-shrink-0">{s.icon}</span>
    ...
    {/* Grid Links */}
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-10 gap-y-10 text-sm">
  ...
  {/* Bottom Bar */}
  <div className="border-t border-gray-700 py-6 px-[5vw] flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 text-sm text-gray-400">
    ...
    {/* Right: Social Icons */}
    <div className="flex items-center gap-4 sm:gap-5 text-lg flex-wrap">
      {socials.map((s, i) => (
        <a
          key={i}
          href={s.url}
