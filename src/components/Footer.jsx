...
const Footer = () => {
 const socials = [
  ...
  { icon: <FaEnvelope />, name: "Mail", handle: "info@divuzl.com", url: "mailto:contact@divuzl.com" },
  {
    icon: (
      <img
        src="public/images/greview.png" // ← replace with your image path or imported image
        alt="Google Reviews"
        className="w-full h-full object-contain"
      />
    ),
    name: "Google Reviews",
    url: "https://share.google/fV8Wrupx5RUO81o8f", // replace with actual link
  },
];
          ...
          <div className="flex flex-wrap gap-4 sm:gap-6 justify-start">
            {socials.map((s, i) => (
              <a
                ...
                title={s.name}
              >
                <span className="text-3xl sm:text-4xl flex justify-center items-center w-full h-full">
                  {s.icon}
                </span>
              </a>
            ))}
